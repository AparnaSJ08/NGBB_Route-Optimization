/**
 * Derives display state from event log at a given frame.
 */
export function deriveFrameState(result, frame) {
  if (!result || !result.events.length) {
    return { currentEvent: null, nodesExplored: 0, bestCost: null, lowerBound: 0,
      gap: null, visitedNodes: new Set(), activeEdges: [], candidateEdge: null,
      prunedNodes: new Set(), isSolved: false, optimalRoute: null, simulatedTimeMs: 0 };
  }
  ...
  return {
    ...
    gap: event.upperBound === Infinity ? null
      : Math.abs(event.upperBound - result.optimalCost) / Math.max(result.optimalCost, 0.01) * 100,
